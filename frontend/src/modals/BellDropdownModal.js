...
const BellDropdownModal = ({ open, anchorRef, notifications = [], setNotifications, setNotificationCount, onClose }) => {
            ...
            {notifications.map(n => (
              <li key={n.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#f7f7fa', borderRadius: 7, marginBottom: 8, padding: '10px 14px', boxShadow: '0 1px 4px #0001' }}>
                ...
                <button
                  ...
                  onClick={e => {
                    e.stopPropagation(); // Evita que se cierre el modal ni se active ningún fondo
