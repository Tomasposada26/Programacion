...
const AuraPanel = ({
  ...
  handleVerify,
  onUserUpdate,
  // Props que aparecían SOLO en la segunda definición duplicada
  handleUserUpdate, // (Alias potencial de onUserUpdate)
  accounts,
  setAccounts
}) => {

  // Estado interno para panel activo si no viene controlado
  ...
  // Ajustes como panel flotante
  const [showSettingsPanel, setShowSettingsPanel] = React.useState(false);
  const ajustesBtnRef = React.useRef(null);

  // Estado animaciones "cards"
  ...
  const renderPanel = () => {
    switch (panelActivo) {
      case 'home':
        return (
          <div style={{ width: '100%', padding: '40px 42px 80px', boxSizing: 'border-box', background: '#fff' }}>
            <div style={{
              width: '100%',
              display: 'flex',
              flexDirection: 'column',
              gap: 0,
              background: 'linear-gradient(135deg, #0a2342 0%, #188fd9 100%)',
              borderRadius: 26,
              padding: '42px 46px 56px',
              boxShadow: '0 14px 42px -8px rgba(10,35,66,0.45)',
              position: 'relative',
              overflow: 'hidden'
            }}>
              {/* Capa de decoración */}
              <div style={{
                position: 'absolute',
                inset: 0,
                background: 'radial-gradient(circle at 20% 30%, rgba(255,255,255,0.08), transparent 60%), radial-gradient(circle at 80% 70%, rgba(255,255,255,0.09), transparent 55%)'
              }} />
              <div style={{ position: 'relative', display: 'flex', gap: 40, zIndex: 2 }}>
                {/* Columna izquierda de bienvenida */}
                <div style={{ flex: 2, display: 'flex', flexDirection: 'column', justifyContent: 'flex-start' }}>
                  {/* Fragmento extraño original conservado como comentario */}
                  {/*
                    </span>
                  */}
                  ...
                  <div style={{ color: '#fff', fontSize: '1.05rem', marginBottom: 18, maxWidth: 1100 }}>
                    En Aura podrás gestionar y automatizar la interacción de tus cuentas de Instagram, descubrir tendencias clave en vacantes, eventos y reuniones,
                    responder automáticamente a tus clientes con Neto nuestro chatbot y obtener una visión clara de tu impacto digital. Además, podrás programar
                    publicaciones, analizar el crecimiento de tus seguidores, recibir alertas inteligentes y mucho más. Todo desde un solo lugar: simple, ágil y seguro.
                    ¡Empieza a explorar y lleva tu presencia digital al siguiente nivel!
                  </div>
                  <div style={{ display: 'flex', gap: 12, marginBottom: 18, flexWrap: 'wrap' }}>
                    <button style={{ background: '#1ee6d9', color: '#183b54', fontWeight: 700, border: 'none', borderRadius: 8, padding: '8px 18px', fontSize: '1rem', cursor: 'pointer' }}>
                      Deja que Neto trabaje por ti 🤖
                    </button>
                    <button style={{ background: '#1ee6d9', color: '#183b54', fontWeight: 700, border: 'none', borderRadius: 8, padding: '8px 18px', fontSize: '1rem', cursor: 'pointer' }}>
                      Tendencias claras, decisiones inteligentes 🧠
                    </button>
                    <button style={{ background: '#1ee6d9', color: '#183b54', fontWeight: 700, border: 'none', borderRadius: 8, padding: '8px 18px', fontSize: '1rem', cursor: 'pointer' }}>
                      Un panel, todas tus cuentas 🗝️
                    </button>
                  </div>
    ...
    //prueba (comentario original preservado)
  };
    ...
    <div className="aura-panel" style={{ display: 'flex', minHeight: '100vh', width: '100vw', background: '#fff' }}>
          ...
          >
            ...
            {(notificationsEnabled
              ...
              <span style={{
                ...
                {notificationsEnabled ? notificationCount : '✕'}
              </span>
            )}
            <BellDropdownModal
      ...
      >
        <div style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: '18px' }}>
          {/* Logo grande (eliminado visualmente en original, se mantiene comentario) */}
        </div>
        <nav style={{ width: '100%' }}>
          <ul style={{ listStyle: 'none', padding: 0, margin: 0, width: '100%' }}>
            <li
              onClick={() => setPanelActivo('home')}
              style={{
                display: 'flex',
                alignItems: 'center',
                gap: '14px',
            ...
            >
              <HomeIcon size={20} color="#fff" /> Inicio
            </li>
            <li
              onClick={() => setPanelActivo('dashboard')}
              style={{
            ...
            >
              <DashboardIcon size={20} color="#fff" /> Dashboard
            </li>
            <li
              onClick={() => setPanelActivo('tendencias')}
              style={{
            ...
            >
              <TrendsIcon size={20} color="#fff" /> Análisis de Tendencias
            </li>
            <li
              onClick={() => setPanelActivo('respuestas')}
              style={{
            ...
            >
              <ChatIcon size={20} color="#fff" /> Respuestas Automáticas
            </li>
            <li
              onClick={() => setPanelActivo('prueba-neto')}
              style={{
                display: 'flex',
                alignItems: 'center',
                gap: '14px',
            ...
            >
              <RobotIcon size={20} color="#fff" /> Prueba Neto
            </li>
            <li
              onClick={() => setPanelActivo('cuentas')}
              style={{
            ...
            >
              <UsersIcon size={20} color="#fff" /> Cuentas
            </li>
          </ul>
        </nav>
        {/* Botón Ajustes */}
        <div style={{
          ...
          <button
            ...
            style={{
              width: '90%',
              background: 'linear-gradient(90deg, #1EE6D9 60%, #188fd9 100%)',
              color: '#fff',
              fontWeight: 700,
              border: 'none',
              borderRadius: '12px',
              ...
              fontSize: '1.08rem',
              cursor: 'pointer',
              boxShadow: '0 2px 8px rgba(30,230,217,0.13)',
      ...
      <ProfileModalWrapper
        ...
        user={user}
        onUserUpdate={onUserUpdate || handleUserUpdate}
      />
      {/* (BellDropdownModal ya está anclado al ícono de campana en el header) */}
    </div>
  );
};
